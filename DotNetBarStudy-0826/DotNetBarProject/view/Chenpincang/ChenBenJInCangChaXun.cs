...
{
    public partial class ChenBenJInCangChaXun : Office2007Form
    {

        private LblWait lblwait;
        public SqlSugarClient db;
        private long SN_SN = -1L;
        public ChenBenJInCangChaXun()
        {
            InitializeComponent();
        ...
        {
            UserProc.CheckTime = "";
            var exp = Expressionable.Create<CP_TABLE>().And(it=>it.status == "1");
        ...
        private string SetDH()
        {
            ...
            string nian = "J"+ nian1 + DateTime.Now.ToString("MM")
        ...
        private void Query(string sn)
        {
            try
            {
                    ...
                    lblKH.cobodgv.Text = a.kehu;
                    riqi.Value = DateTime.Now;
                    lblPM.cobodgv.Text = a.peiming;
                    //lblCW.txt.Text = a.cangwei;
                    lblYWY.cobodgv.Text = a.yewuyuan;
        ...
        private void EnableInput(bool index)
        {
            lblGH.txt.Enabled = index;
            lblRKDH.txt.Enabled = index;
            lblKH.cobodgv.Enabled = index;
            riqi.Value = DateTime.Now;
            lblPM.cobodgv.Enabled = index;
            lblCW.txt.Enabled = index;
            lblYWY.cobodgv.Enabled = index;
            lblDDH.txt.Enabled = index;
            lblPS.txt.Enabled = index;
            lblJCY.cobodgv.Enabled = index;
            lblZL.txt.Enabled = index;
            lblBZ.txt.Enabled = index;
        }
        private bool Save()
        {
            ...
            {
                var aa = db.Queryable<CP_TABLE>().Where(it => it.kehu == lblGH.txt.Text && it.status == "1").ToList();
                int d = 0;
                for(var i = 0; i < aa.Count; i++)
                {
                    d += Convert.ToInt32(aa[i].pishu);
                }
                if((d + Convert.ToInt32(lblPS.txt.Text)) > Convert.ToInt32(all1.peishu))
                {
                    MessageBoxEx.Show("入仓匹数超过总匹数，总匹数:"+ all1.peishu, "提示");
                    return false;
                }
        ...
        private bool Edit()
        {
            var asd = db.Queryable<CP_TABLE>().Where(it => it.SN == SN_SN && it.status == "1").First();
            if (asd == null)
            {
            ...
            else
            {
                ...
                asd.ganghao = lblGH.txt.Text;
                asd.rukudanhao = lblRKDH.txt.Text;
                asd.kehu = lblKH.cobodgv.Text;
                asd.riqi = riqi.Value;
                asd.pinming = lblPM.cobodgv.Text;
                asd.cangwei = lblCW.txt.Text;
                asd.yewuyuan = lblYWY.cobodgv.Text;
                asd.dingdanhao = lblDDH.txt.Text;
                asd.pishu = lblPS.txt.Text;
                asd.jincangyuan = lblJCY.cobodgv.Text;
                asd.zhongliang = lblZL.txt.Text;
                asd.beizu = lblBZ.txt.Text;
        ...
        private void 打印ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            this.lblwait.showme();
            this.dgvEX1.Print("成品进仓", UserProc.CheckTime);
            this.lblwait.hideme();
        }
