...
{
    public partial class GreigeName : Office2007Form
    {

        string SHAZHONG = leibie.enumLB.纱种.ToString();
        ...
        private List<T_Base> schText()
        {
            ...
            var exp = Expressionable.Create<T_Base>().And(it => it.leibie == SHAZHONG);
                //.And(it => it.Id == 1)
                //.ToExpression();//拼接表达式
        ...
        private bool editRow()
        {
            ...
            { 
                ...
                var get = db.Queryable<T_Base>().Where(it => it.leibie == SHAZHONG && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
        ...
        private void PopRow()
        {
            SN_SN = Convert.ToInt64(dgvEX1.Rows[dgvEX1.CurrentRow.Index].Tag);
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == SHAZHONG && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
        ...
        private void 导出ExcleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            lblwait.showme();
            this.dgvEX1.saveExcel(SHAZHONG);
            lblwait.hideme();
        }
        ...
        private void GreigeName_Load(object sender, EventArgs e)
        {
            Column2.Tag = "总行:";
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == SHAZHONG)
