...
{
    public partial class RanHuaType : Office2007Form
    {
        string RANHUALIEBIE = leibie.enumLB.染化类别.ToString();
        ...
        private List<T_Base> schText()
        {
            ...
            var exp = Expressionable.Create<T_Base>().And(it => it.leibie == RANHUALIEBIE);
                //.And(it => it.Id == 1)
                //.ToExpression();//拼接表达式
        ...
        private bool Insert()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == RANHUALIEBIE && it.bianhao == lblTxt1.txt.Text).Count();
                if (get != 0)
                {
        ...
        private bool editRow()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == RANHUALIEBIE && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
        ...
        private void PopRow()
        {
            ...
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == RANHUALIEBIE && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
        ...
        private void 打印ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            this.lblwait.showme();
            this.dgvEX1.Print(RANHUALIEBIE, "");
            this.lblwait.hideme();
        }
        ...
        private void 导出ExcleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            lblwait.showme();
            this.dgvEX1.saveExcel(RANHUALIEBIE);
            lblwait.hideme();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == RANHUALIEBIE)
                .ToList();
            ShowData(getAll);
        ...
        private void RanHuaType_Load(object sender, EventArgs e)
        {
