...
{
    public partial class EmployeeName : Office2007Form
    {
        string YUANGONG = leibie.enumLB.员工.ToString();
        ...
        private void ShowData(List<T_Base> getAll)
        {
            ...
            for (var i = 0; i < getAll.Count; i++)
            {
                ...
                dgvEX1.Rows[index].Cells[3].Value = dataRowCollection.item1;
        ...
        private bool Insert()
        {
            ...
            try
            {
                ...
                var get = db.Queryable<T_Base>().Where(it => it.leibie == YUANGONG && it.bianhao == lblTxt1.txt.Text).Count();
                if (get != 0)
                {
        ...
        private bool editRow()
        {
            ...
            try
            {
                ...
                var get = db.Queryable<T_Base>().Where(it => it.leibie == YUANGONG && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
                ...
                var das = db.Queryable<T_Base>().Where(it => it.SN == SN_SN).First();
                if(das != null)
                {
                    das.bianhao = lblTxt1.txt.Text;
                    das.itemName = lblTxt2.txt.Text;
                    das.item0 = lblTxt3.txt.Text;
                    das.item1 = lblTxt4.txt.Text;

                    var t2 = db.Updateable<T_Base>(das).ExecuteCommand();
                    flag = true;
                }
            }
            catch(Exception ex)
        ...
        {
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == YUANGONG && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
        ...
        private void 导出ExcleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            lblwait.showme();
            this.dgvEX1.saveExcel(YUANGONG);
            lblwait.hideme();
        }
