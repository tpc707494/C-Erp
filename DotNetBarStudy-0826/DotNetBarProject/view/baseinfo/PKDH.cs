...
{
    public partial class PKDH : Office2007Form
    {
        string CW = leibie.enumLB.胚客代号.ToString();
        ...
        public PKDH()
        {
            ...
            /*
            var width = (ScreenArea.Width) / dgvEX1.Columns.Count;
            for (var i=0;i< dgvEX1.Columns.Count; i++)
            {
                var column = dgvEX1.Columns[i];
                column.Width = width;
            }
            */
            ...
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == CW)
                .ToList();
            ShowData(getAll);
            chkTxt3.cobodgv.RefList(coboDGV.leibie.客户);
            lblTxt1.RefList(coboDGV.leibie.客户);
        }
        ...
        private List<T_Base> schText()
        {
            ...
            var exp = Expressionable.Create<T_Base>().And(it => it.leibie == CW);
                //.And(it => it.Id == 1)
                //.ToExpression();//拼接表达式
            if (chkTxt1.chk.Checked)
            {
                exp.And(it => it.bianhao.Contains(chkTxt1.txt.Text));
            }
            if (chkTxt2.chk.Checked)
            {
                exp.And(it => it.itemName.Contains(chkTxt2.txt.Text));
            }
        ...
        private bool Insert()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == CW && it.bianhao == lblTxt3.txt.Text).Count();
                if (get != 0)
                {
                ...
                {
                    leibie = CW,
                    bianhao = lblTxt3.txt.Text,
                    itemName = lblTxt2.txt.Text,
                    item0 = lblTxt1.cobodgv.Text,
        ...
        private bool editRow()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == CW && it.bianhao == lblTxt3.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
                ...
                if (dsa != null)
                {
                    dsa.bianhao = lblTxt3.txt.Text;
                    dsa.itemName = lblTxt2.txt.Text;
                    dsa.item0 = lblTxt1.cobodgv.Text;
        ...
        private void EmptyRow()
        {
            ...
            this.lblTxt1.cobodgv.Text = "";
        }
        private void PopRow()
        {
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == CW && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
            ...
            lblTxt3.txt.Text = current.bianhao;
            lblTxt1.cobodgv.Text = current.item0;
        ...
        private void Cangwei_Load(object sender, EventArgs e)
        {
            Column2.Tag = "总行:";
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == CW)
