...
{
    public partial class RanHuaName : Office2007Form
    {
        string RANLIAOMINGCHEN = leibie.enumLB.染料名称.ToString();
        ...
        public RanHuaName()
        {
            ...
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == RANLIAOMINGCHEN)
                .OrderBy(it=>it.bianhao, OrderByType.Asc)
                .ToList();
            ShowData(getAll);
        }
        /*
        ...
        */
        ...
        private void 打印ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            this.lblwait.showme();
            this.dgvEX1.Print(RANLIAOMINGCHEN, "");
            this.lblwait.hideme();
        }
        //导出按钮
        private void 导出ExcleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            lblwait.showme();
            this.dgvEX1.saveExcel(RANLIAOMINGCHEN);
            lblwait.hideme();
        }
        //查询按钮
        ...
        private List<T_Base> schText()
        {
            ...
            {
                var exp = Expressionable.Create<T_Base>().And(it => it.leibie == RANLIAOMINGCHEN);
                //.And(it => it.Id == 1)
                //.ToExpression();//拼接表达式
                if (chkBianhao.chk.Checked)
                {
                ...
                if (chkType.chk.Checked)
                {
                    exp.And(it => it.itemName == chkType.cobodgv.Text);
        ...
        private bool Insert()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == RANLIAOMINGCHEN && it.bianhao == lblTxt1.txt.Text).Count();
                if (get != 0)
                {
                ...
                {
                    leibie = RANLIAOMINGCHEN,
                    bianhao = lblTxt1.txt.Text,
                    itemName = lblTxt2.cobodgv.Text,
        ...
        private bool editRow()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == RANLIAOMINGCHEN && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
                }
                var asdf = db.Queryable<T_Base>().Where(it => it.SN == SN_SN).First();
                if (asdf != null)
                {
                    asdf.bianhao = lblTxt1.txt.Text;
                    asdf.itemName = lblTxt2.cobodgv.Text;
                    asdf.item0 = lblTxt3.txt.Text;
                    asdf.item1 = lblTxt4.txt.Text;
                    asdf.item2 = lblTxt5.txt.Text;
                    asdf.item3 = lblTxt6.txt.Text;
                    asdf.riqi = DateTime.Now;
        ...
        private void PopRow()
        {
            SN_SN = Convert.ToInt64(dgvEX1.Rows[dgvEX1.CurrentRow.Index].Tag);
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == RANLIAOMINGCHEN && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
            ...
            lblTxt2.cobodgv.Text = current.itemName;
        ...
        private void RanHuaName_Load(object sender, EventArgs e)
        {
            Column2.Tag = "总行:";
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == RANLIAOMINGCHEN)
        ...
        {
            lblTxt2.RefList(coboDGV.leibie.染化类别);
            chkType.cobodgv.RefList(coboDGV.leibie.染化类别);
        }
    }
}
