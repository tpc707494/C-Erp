...
{
    public partial class ColorType : Office2007Form
    {
        string SEHAOLEIBIE = leibie.enumLB.色号类别.ToString();
        ...
        public ColorType()
        {
            ...
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == SEHAOLEIBIE)
                .ToList();
            ShowData(getAll);
        }
        ...
        private List<T_Base> schText()
        {
            ...
            var exp = Expressionable.Create<T_Base>().And(it => it.leibie == SEHAOLEIBIE);
                //.And(it => it.Id == 1)
                //.ToExpression();//拼接表达式
        ...
        private bool Insert()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == SEHAOLEIBIE && it.bianhao == lblTxt1.txt.Text).Count();
                if (get != 0)
                {
        ...
        private bool editRow()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == SEHAOLEIBIE && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
        ...
        private void PopRow()
        {
            ...
            var getAll = db.Queryable<T_Base>()
                   .Where(it => it.leibie == SEHAOLEIBIE && it.SN==SN_SN)
                   .ToList();
            if(getAll.Count != 1)
            {
        ...
        private void 导出ExcleToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.dgvEX1.Rows.Count == 0)
                return;
            lblwait.showme();
            this.dgvEX1.saveExcel(SEHAOLEIBIE);
            lblwait.hideme();
        }
        ...
        private void ColorType_Load(object sender, EventArgs e)
        {
            Column2.Tag = "总行:";
            var getAll = db.Queryable<T_Base>()
                .Where(it => it.leibie == SEHAOLEIBIE)
