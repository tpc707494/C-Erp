...
{
    public partial class Cangwei : Office2007Form
    {
        string CW = leibie.enumLB.仓位.ToString();
        ...
        private bool Insert()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == CW && it.bianhao == lblTxt1.txt.Text).Count();
                if (get != 0)
                {
        ...
        private bool editRow()
        {
            ...
            try
            {
                var get = db.Queryable<T_Base>().Where(it => it.leibie == CW && it.bianhao == lblTxt1.txt.Text && (it.SN != SN_SN)).Count();
                if (get != 0)
                {
                    int num = (int)MessageBox.Show(lblTxt1.lblText + " 已存在!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                    return flag;
