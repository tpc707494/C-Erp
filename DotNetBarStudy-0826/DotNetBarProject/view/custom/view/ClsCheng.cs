...
{
    internal partial class ClsCheng : SerialPort
    {
        public Decimal? _showVal = new Decimal?();
        public Decimal? _maoVal = new Decimal?();
        public bool _WenDing = false;
        ...
        public ClsCheng(IContainer container)
        {
            container.Add(this);
        ...
        private void getVal_ZHUOJING()
        {
            ...
            if (num >= 0 && startIndex >= 0)
            {
                string str2 = str1.Substring(startIndex, 11);
                this.WenDing = str2.StartsWith("S");
        ...
        public bool setZERO()
        {
            if (!this.IsOpen)
            {
                int num = (int)MessageBoxEx.Show("串口未打开!!!", "提示");
                return false;
            }
            switch (ChengPara.chengXH)
            {
