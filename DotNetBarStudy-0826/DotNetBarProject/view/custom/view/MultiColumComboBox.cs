...
namespace DotNetBarProject.view.custom.view
{
    ...
    public partial class MultiColumComboBox : UserControl
    {
        ...
        public delegate void DoubleList(string asd, string cou2);
        ...
        private string _allFiled = "";
        public string AllFiled
        {
            get { return _allFiled; }
            set { _allFiled = value; }
        }
        ...
        private void ConstructDataColumns()
        {
            if (_SourceDataString != null)
            {
                ...
                for (int index = 0; index < _SourceDataString.Length; index++)
                {
                    ...
                    if (_ColumnWidth != null && _ColumnWidth.Count > index)
                    {
                        int colWidth;
        ...
        private void CalculateDropDownWidth()
        {
            ...
            {
                for (int index = 0; index < _ColumnWidth.Count; index++)
                {
        ...
        private void SetFilter(string filterText)
        {
            ...
            try
            {
                ...
                {
                    if (AllFiled.Length != 0)
                    {
                        string text = "";
                        string[] array = this._allFiled.Split(new char[]
                        {
                            ','
                        });
                        string[] array2 = array;
        ...
        private void InitializeMultiColumnGridView()
        {
            ...
            this.dgMultiColumn.CellClick += new System.Windows.Forms.DataGridViewCellEventHandler(this.dgMultiColumn_CellDoubleClick);
        ...
        private void dgMultiColumn_ItemSeleted(DataGridViewRow item)
        {
            ...
            else
            {
                ...
                {
                    doublelist?.Invoke(item.Cells[0].Value.ToString(), "");
                }
                else
                {
                    doublelist?.Invoke(item.Cells[0].Value.ToString(), item.Cells[1].Value.ToString());
                }
            }
            popup.Close();
        }
