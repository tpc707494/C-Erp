...
{
    public partial class DuiZhangDanMake : Office2007Form
    {
        ...
        private void Txt_Change(object sender, EventArgs e)
        {
            try
            {
                float dj = 0;
                float zz = 0;
                if(lblPIH.txt.Text.Length>0)
                dj = float.Parse(lblPIH.txt.Text); 

                if(lblZZ.txt.Text.Length>0)
                zz = float.Parse(lblZZ.txt.Text); 

                lblJE.txt.Text =  ( dj * zz) .ToString() ;
            }
            catch (Exception ex)
            {
                MessageBoxEx.Show("单价或重量输入非法！");
                return;
            }
        }
